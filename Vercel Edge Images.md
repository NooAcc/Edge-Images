# Vercel Node.js 图片处理程序需求文档（基于 @cf-wasm/photon）
...
- 采用纯 WASM 图像处理库 `@cf-wasm/photon`，避免原生依赖，保证跨平台兼容和简易部署。
...
- **关键限制**：
  ...
  - 每月调用次数：100 万次。
  - 每月 CPU 时间：4 小时。
- **技术约束**：Node.js Runtime 允许使用所有 Node.js API，但本方案特意选择纯 WASM 库，以确保极致轻量、快速的冷启动和未来向 Edge Runtime 迁移的能力。
...
2. 源图片获取：使用 `fetch` 下载第三方图片，设置超时 8 秒。若下载失败返回 502。
3. 图片处理：
   - 解码源图片为 Photon 可处理的图像对象。
   - 应用旋转/翻转（如果指定）。
   - 根据 `fit` 模式和目标尺寸执行缩放、裁剪或填充。
   - 调用 Photon 的 WebP 编码器，传入质量参数（默认 85）输出 WebP 字节流。
4. 响应构造：
...
## 4. 技术选型与理由
- **运行时**：Node.js Runtime（Vercel Serverless Functions）。提供充裕的 1024 MB 内存和 50 MB 包体积，让我们可以轻松容纳 WASM 文件，毫无压力。
- **图像处理库**：`@cf-wasm/photon`。
  - 基于 Rust 编译的 WebAssembly，无原生依赖，`npm install` 即可，跨平台部署无忧。
  - 功能丰富：内置缩放、裁剪、旋转、翻转、色彩填充等能力，完全覆盖我们的 fit 模式需求。
  - 性能优异：比 JavaScript 实现快 4-10 倍，虽稍慢于 `sharp`，但对目标场景性能绰绰有余。
  - 单一 WASM 文件，压缩后约 2.5 MB，在 50 MB 限制下非常安全。
  - 内存占用可预测且较低，在 Hobby 计划 1024 MB 限制下几乎无溢出风险。
  - 具备高可移植性：未来若想迁移到 Edge Runtime 或 Cloudflare Workers，代码几乎无需改动。

### 参考开源项目
我们将基于社区项目 `yoeven/next-image-processing-api`（已使用 `@cf-wasm/photon`）进行改造，该仓库提供了参数解析、缩放裁剪逻辑的骨架，但缺少 WebP 质量控制、部分 fit 模式和分辨率上限，这些将在本需求中明确要求实现。
...
│ └── process-image.js # 图片处理管道（@cf-wasm/photon 调用封装）
├── package.json
...
### 5.2 `fetch-image.js`
- 使用 `fetch` 和 `AbortController` 实现超时（8 秒）。
- 若响应状态非 200，抛出错误。
- 返回响应的 `Buffer`（通过 `arrayBuffer()` 然后 `Buffer.from()`），因为 Photon 接口可直接从字节数组创建图像。

### 5.3 `process-image.js`
- 接收源图片 Buffer 和参数对象。
- 使用 `@cf-wasm/photon` 的 `PhotonImage.new_from_byteslice()` 创建图像对象。
- 应用旋转变换（`flip_horizontal`, `flip_vertical`, `rotate90` 等函数）。
- 根据 `fit` 模式计算目标尺寸并执行：
...
- 调用图像对象的 `get_bytes_webp(quality)` 方法，传入质量参数，输出 WebP 字节。
- 调用 `image.free()` 释放内存（非常重要，避免内存泄漏）。
- 返回 WebP 字节数组。
...
- Photon 处理失败（如内存不足） → 若源图已获取，则返回原始图片（200 OK），并在 `X-Processing-Error` 头中记录错误信息，确保用户体验不受损。
- 任何未捕获异常 → 500 `Internal Server Error`。

## 8. 性能目标与资源监控
- **首次冷启动**（含 WASM 加载）：期望 < 500 ms。
...
- **依赖管理**：`@cf-wasm/photon` 必须作为 `dependency` 安装在 `package.json` 中，严禁使用全局安装等非标准方式。
...
## 11. 关键代码改造清单（基于 yoeven/next-image-processing-api）

1. **WebP 质量参数**：修改 `photon` 调用中的 `get_bytes_webp()`，传入 `quality` 参数。
...
4. **移除商业 API 调用**：清理任何指向外部商业服务的逻辑，仅使用本地 Photon。
5. **内存管理**：确保每次处理后在 finally 块中调用 `image.free()`。
6. **响应头设置**：正确设置 `Content-Type` 和 `Cache-Control`。