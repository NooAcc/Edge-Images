...
- 可选参数（使用 sharp 原生参数语义）：
  - `width`：目标宽度（像素），最大 1024。若仅提供宽度，高度按比例缩放。
  - `height`：目标高度（像素），最大 1024。若仅提供高度，宽度按比例缩放。
  - `fit`：缩放模式，支持：
    - `inside`：保持比例缩小到目标框内，不放大（默认）。
    - `cover`：使用 sharp 原生 cover 模式填满目标框并裁剪溢出。
    - `contain`：保持宽高比完整包含在目标框内，空白部分由背景色填充（默认白色）。
    - `fill`：直接拉伸到目标尺寸，不保持原始宽高比。
    - `outside`：保持比例缩放到至少覆盖一个目标维度，不裁剪。
  - `quality`：WebP 输出质量，范围 1-100，**默认 85**。该参数直接影响有损压缩程度。
  - `format`：输出格式，当前版本仅支持 `webp`（默认）。预留扩展。
  - `background`：填充背景色，十六进制 RRGGBB，默认 `FFFFFF`。适用于 `fit=contain` 模式。
...
### 3.3 输出要求
- 最终输出图片分辨率 **严格不超过 1024×1024**。若未指定尺寸但原图超过该限制，应强制缩小（通过 fit 默认模式 `inside`）。
...
- 接收源图片 Buffer 和参数对象。
- 使用 sharp 从源图片 Buffer 创建原生处理管线。
- 应用旋转和翻转变换。
- 直接调用 sharp 原生 `resize()`：
  - `inside`：保持比例缩小到目标框内，不放大。
  - `cover`：填满目标框并裁剪溢出。
  - `contain`：完整包含并使用背景色填充空白。
  - `fill`：直接拉伸到目标尺寸。
  - `outside`：保持比例缩放到至少覆盖一个目标维度。
...
| 填充模式 `fit=contain` | `width=500&height=500&fit=contain&background=FF0000` | 图像完整呈现，周围填充红色 (#FF0000) |
| 缩小模式 `inside` | 原图 500×500，传 `width=1024&height=1024&fit=inside` | 图片不放大，输出保持 500×500 |
| 质量参数 | `quality=50` | 输出 WebP 文件明显压缩，肉眼可见块效应 |
| 透明通道 | 源图为透明 PNG | 输出 WebP 透明区域保持透明 |
| 超宽原图限制 | 原图 5000×5000，不传宽高 | 输出时自动缩小到 1024×1024 以内（fit 默认为 inside） |
...
2. **使用 sharp 原生 fit 模式**：直接支持 `cover`、`contain`、`fill`、`inside`、`outside`。
