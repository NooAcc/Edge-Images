...
export function parseParams(sourceUrl, query, options = {}) {
  ...
  const maxDimension = platformConfig?.maxDimension ?? 1024;
  const defaultQuality = platformConfig?.defaultQuality ?? 85;

  const width = parseDimension(getQueryValue(query, 'width'), 'width', maxDimension);
  const height = parseDimension(getQueryValue(query, 'height'), 'height', maxDimension);
  const quality = parseQuality(getQueryValue(query, 'quality'), defaultQuality);
...
function parseDimension(rawValue, name, maxDimension) {
...
function parseQuality(rawValue, defaultQuality) {
  if (rawValue === undefined || rawValue === '') {
