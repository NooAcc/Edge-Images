...
export const SUPPORTED_FITS = new Set(["scale-down", "contain", "cover", "pad", "crop"]);
export const SUPPORTED_FORMATS = new Set(["webp"]);
...
export function parseParams(query, options = {}) {
  const sourceUrl = getQueryValue(query, "url");
  if (!sourceUrl) {
    throw new ParamError("Missing required parameter: url");
  }
  ...
  if (!isUrlAllowed(url, urlAllowlist)) {
    throw new ParamError(`url host is not allowed by ${options.allowlistEnvName || "IMAGE_URL_ALLOWLIST"}`);
  }
...
function parseFit(rawValue) {
  ...
  return SUPPORTED_FITS.has(value) ? value : DEFAULT_FIT;
}
...
function parseFormat(rawValue) {
  ...
  if (!SUPPORTED_FORMATS.has(value)) {
    throw new ParamError("format must be webp");
  }
