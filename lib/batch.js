...
export function createBatchHandler({
  ...
  return async function batchHandler(req, res) {
    ...
    for (const item of body) {
      ...
      if (cache) {
        ...
        if (cached) {
          ...
          const contentType = cached.header?.contentType || 'application/octet-stream';
          response[uuid] = { success: true, data: { base64, contentType } };
          continue;
        }
      }
    ...
    // Phase 1: Download unique sources concurrently, deduplicated by URL
    const urlGroups = new Map();
    for (const entry of pending) {
      const key = entry.params.url;
      if (!urlGroups.has(key)) urlGroups.set(key, []);
      urlGroups.get(key).push(entry);
    }

    const sourceCache = new Map();
    await Promise.all(
      [...urlGroups.keys()].map(async (url) => {
        const entries = urlGroups.get(url);
        const isVideo = entries[0].isVideo;

        if (cache) {
          const sourceKey = buildCacheKey('source', url);
          const cached = await cache.get(sourceKey);
          if (cached) {
            sourceCache.set(url, {
              buffer: cached.buffer,
              contentType: cached.header?.contentType || 'application/octet-stream',
            });
            return;
          }
        }
        ...
        try {
          ...
          if (isVideo) {
            buffer = await extractVideoFrameRange(url, { logger: requestLogger });
            contentType = 'image/png';
          } else {
            const source = await fetchImage(url, { logger: requestLogger });
            buffer = source.buffer;
            contentType = source.contentType;
          }

          sourceCache.set(url, { buffer, contentType });
    ...
    // Distribute sources and mark failures in one pass
    const toProcess = [];
    for (const entry of pending) {
      const source = sourceCache.get(entry.params.url);
      if (source) {
      ...
      toProcess.map((entry) =>
        globalLimiter(async () => {
          ...
          try {
            ...
            const contentType = FORMAT_CONTENT_TYPES[params.format] || 'application/octet-stream';
            response[uuid] = { success: true, data: { base64, contentType } };
          } catch (error) {
            ...
            response[uuid] = { success: true, data: { base64, contentType: sourceContentType } };
          }
        }),
      ),
    );
