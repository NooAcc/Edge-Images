...
import { buildImageFetchHeaders, parseSourceSize } from './fetch-image.js';
...
function shouldUseSystemFfmpeg() {
  const env = process.env.USE_SYSTEM_FFMPEG;
  return env === undefined || env === '' || env === 'true';
}

export async function getFfmpegPath() {
  if (shouldUseSystemFfmpeg() && process.env.FFMPEG_PATH) return process.env.FFMPEG_PATH;
...
export async function getFfprobePath() {
  if (shouldUseSystemFfmpeg() && process.env.FFPROBE_PATH) return process.env.FFPROBE_PATH;
...
export async function probeVideoMetadataFromUrl(url, options = {}) {
  ...
  const source = await fetchWithRangeFallback(url, {
    fetchImpl,
    rangeSize,
    logger,
    timeoutMs,
  ...
  try {
    const metadata = await probeVideoMetadata(source.buffer, {
      ffprobePath,
    ...
    return { ...metadata, sourceSize: source.sourceSize };
  } catch {
    if (source.isCompleteDownload) {
    ...
    const fullSource = await fetchFull(url, { fetchImpl, logger, timeoutMs });
    const metadata = await probeVideoMetadata(fullSource.buffer, {
      ffprobePath,
      runProcess: runProcessFn,
      timeoutMs,
      logger,
    ...
    logger.info('video.probe.done', {
      ...metadata,
      bytesRead: fullSource.bytesRead,
      sourceSize: fullSource.sourceSize,
      durationMs: Date.now() - startedAt,
    });

    return { ...metadata, sourceSize: fullSource.sourceSize };
  }
}
...
export async function extractVideoFrameRange(url, options = {}) {
  ...
  try {
    const frame = await extractVideoFrame(source.buffer, {
      ffmpegPath,
      runProcess: runProcessFn,
      timeoutMs,
      logger,
    ...
    const fullSource = await fetchFull(url, { fetchImpl, logger, timeoutMs });
    const frame = await extractVideoFrame(fullSource.buffer, {
      ffmpegPath,
      runProcess: runProcessFn,
      timeoutMs,
      logger,
...
async function extractVideoFrame(sourceBuffer, options = {}) {
  ...
  const args = ['-i', 'pipe:0', '-vframes', '1', '-f', 'image2', '-vcodec', 'png', '-y', 'pipe:1'];
...
async function fetchWithRangeFallback(url, options = {}) {
  const {
    ...
    rangeSize = DEFAULT_VIDEO_RANGE_SIZE,
  ...
  try {
    ...
    if (response.status === 206) {
      ...
      return {
        ...
        sourceSize,
        isCompleteDownload: sourceSize !== null && buffer.length >= sourceSize,
      };
    }
    ...
    const arrayBuffer = await response.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);
    const sourceSize = parseSourceSize(response.headers, { allowContentLength: true });

    logger.info('video.fetch.full', {
      url,
      bytes: buffer.length,
      sourceSize: sourceSize ?? buffer.length,
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeNextChunk() {
      ...
      while (offset < stdinBuffer.length && !stdinClosed) {
        const chunk = stdinBuffer.subarray(
          offset,
          Math.min(offset + CHUNK_SIZE, stdinBuffer.length),
        );
